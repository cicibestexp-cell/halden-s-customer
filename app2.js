...
async function renderProfileReservations() {
  const activeList = document.getElementById('prof-active-list');
  const pastList = document.getElementById('prof-past-list');
  if (!activeList || !pastList) return;

  activeList.innerHTML = '<div style="padding:20px;text-align:center;">Loading reservations...</div>';
  pastList.innerHTML = '';
  ...
  try {
    ...
    userReservations.forEach(res => {
      const isPast = ['completed', 'cancelled'].includes(res.status.toLowerCase());
      const cardHtml = `
        <div class="prof-card">
          <div class="prof-card-hdr">
            <div class="prof-card-type">${res.type}</div>
            <div class="prof-card-status ${res.status.toLowerCase()}">${res.status}</div>
          </div>
          <div class="prof-card-date">📅 ${res.date}</div>
    ...
    activeList.innerHTML = activeHtml || '<div class="prof-empty">No active reservations. Imagine your next event! ✦</div>';
    pastList.innerHTML = pastHtml || '<div class="prof-empty">No past events yet.</div>';

  } catch (err) {
    console.error(err);
    activeList.innerHTML = '<div class="prof-empty" style="color:var(--red);">Failed to load history.</div>';
  }
}
